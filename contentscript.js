...
function search(input, head, pickedCourses) {
    ...
    for (var i = 0; i < positionArr.length; i += 1) {
        ...
        rowEl[i].addEventListener("click", function() {
            selectCourse(head, pickedCourses, this.id);
        });
    }
...
function selectedCoursesTable(head, pickedCourses) {
    ...
    for (var i = 0; i < pickedCourses.size; i += 1) {
        ...
        rowEl[i].addEventListener("click", function() {
            deSelectCourse(head, pickedCourses, this.id);
        });
    }
...
function clrUpdateCalendar(pickedCourses) {
    ...
    while (courseslots.hasChildNodes()) {
        courseslots.removeChild(courseslots.childNodes[0]);
    }
...
function updateCalendar(pickedCourses) {
    ...
    while (curr !== null) {
        tableSlots = tableSlots.concat(getAllSlots(getAllTimes(curr.course), curr.course));
        curr = curr.next;
    }
...
function getAllSlots(allTimes, course) {
    var allSlots = [];
    if ("lecTime" in allTimes) {
        allSlots = allSlots.concat(getSingleSlots(allTimes.lecTime, course, "lecTime"));
    }
    if ("labTime" in allTimes) {
        allSlots = allSlots.concat(getSingleSlots(allTimes.labTime, course, "labTime"));
    }
    if ("semTime" in allTimes) {
        allSlots = allSlots.concat(getSingleSlots(allTimes.semTime, course, "semTime"));
    }
    return allSlots;
}

//not yet fully implemented
function getSingleSlots(time, course, timeType) {
    var slots = [];
    time.days.forEach(function(day) {
        ...
        ypos = time.timeStart - 980;
        //duration = (endTime - startTime)*yHeight/yScale;
        duration = (time.timeEnd - time.timeStart)/yScale;
        // FIXED SIZES ***NOTE
        slotDiv.style.left = xpos + 'px';
        slotDiv.style.top = ypos + 'px';
        slotDiv.style.height = duration + 'px';
        slotDiv.style.width = xWidth + 'px';
        slotDiv.style.position = 'absolute';
        slotDiv.style.border = '1px solid';

        slotDiv.style.textAlign = "center";
        slotDiv.style.fontSize = "1 vh";
        slotDiv.innerHTML += course.name;
        if (timeType === "lecTime") {
            slotDiv.innerHTML += "<br>LEC";
        } else if (timeType === "labTime") {
            slotDiv.innerHTML += "<br>LAB";
        } else if (timeType === "semTime") {
            slotDiv.innerHTML += "<br>SEM";
        }
        slots.push(slotDiv);
    });
    return slots;
